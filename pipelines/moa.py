...
class Pipeline:
    class Valves(BaseModel):
        ...
        dekallm_api_base: str = Field(
            default="https://dekallm.cloudeka.ai/v1",
    ...
    def pipe(
        ...
        try:
            self.validate_models(__event_emitter__)
        except ValueError as e:
            self.emit_status(__event_emitter__, "error", str(e), True)
            raise ValueError({"error": str(e)})

        if not messages:
            error_msg = "No messages found in the request body"
            self.emit_status(__event_emitter__, "error", error_msg, True)
            raise ValueError({"error": error_msg})

        last_message = messages[-1]["content"]
        moa_response = self.moa_process(last_message, __event_emitter__)

        if moa_response.startswith("Error:"):
            self.emit_status(__event_emitter__, "error", moa_response, True)
            raise ValueError({"error": moa_response})
    ...
    def validate_models(
        self, __event_emitter__: Callable[[dict], None] = None
    ):
        self.emit_status(__event_emitter__, "info", "Validating models", False)
        valid_models = []
        for model in self.valves.models:
            response = self.query_ollama(model, "Test prompt", __event_emitter__)
    ...
    def moa_process(
        self, prompt: str, __event_emitter__: Callable[[dict], None] = None
    ) -> str:
        if (
            not self.valves.models
            or not self.valves.aggregator_model
            or not self.valves.dekallm_api_base
        ):
        ...
        for layer in range(self.valves.num_layers):
            ...
            current_layer_outputs = [task for task in tasks]
    ...
    def process_agent(
        ...
        if layer == 0:
            response = self.query_ollama(agent, prompt, __event_emitter__)
        else:
            self.emit_status(
                __event_emitter__,
                "info",
    ...
    def query_ollama(
        ...
        url = f"{self.valves.dekallm_api_base}/chat/completions"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.valves.dekallm_api_key}"
        }
        ...
        try:
            ...
            error_message = f"HTTP error querying Ollama API for model {model}: {e.response.status_code}, {e.response.reason}"
            self.emit_status(__event_emitter__, "error", error_message, True)
            print(error_message)
            return f"Error: Unable to query model {model} due to HTTP error {e.response.status_code}"
    ...
    def emit_status(
        self,
        __event_emitter__: Callable[[dict], None],
        level: str,
        message: str,
        done: bool,
    ):
